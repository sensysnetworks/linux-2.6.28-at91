...
static u8 *rdac_spi_cs = NULL;

static int rdac_set_gpio_value(unsigned pin, int value)
{
	if (!rdac_spi_cs)
		return 0;

	if (value)
		*rdac_spi_cs |= 1 << (pin - RDAC_PIN_BASE);
	else
		*rdac_spi_cs &= ~(1 << (pin - RDAC_PIN_BASE));
	return 0;
}

static int rdac_get_gpio_value(unsigned pin)
{
	if (!rdac_spi_cs)
		return 0;

	return ((*rdac_spi_cs & (1 << (pin - RDAC_PIN_BASE))) ? 1 : 0);
}

void rdac_init_spi_cs(u8 *rdac_spi)
{
	rdac_spi_cs = rdac_spi;
}
...
/** EMAC GPIO devices and other drivers may use these outside of __init
 * context for runtime configuration. As such, the __init_or_module qualifiers
 * have been removed.
 */
...
#ifdef CONFIG_RDAC_CARRIER
	if (pin >= RDAC_PIN_BASE)
		return rdac_set_gpio_value(pin, value);
#endif
...
#ifdef CONFIG_RDAC_CARRIER
	if (pin >= RDAC_PIN_BASE)
		return rdac_get_gpio_value(pin);
#endif
