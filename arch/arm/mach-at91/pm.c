...
{
#if defined(CONFIG_SOM_9260_1K_TC) || defined(CONFIG_RDAC_CARRIER)
#define RC_VAL 18
	u8 rc_div;
#define at91_tc_write(reg, val)	__raw_writel((val), reg)
	unsigned long tc;
#ifdef CONFIG_RDAC_CARRIER
	tc = (unsigned long)ioremap(AT91SAM9260_BASE_TC2, SZ_16K);
#else
	tc = (unsigned long)ioremap(AT91SAM9260_BASE_TC1, SZ_16K);
#endif
#endif
	...
	switch (state) {
		...
		case PM_SUSPEND_MEM:
			...
			if (slow_clock) {
				...
				at91_tc_write((tc + AT91_TC_RC), RC_VAL /
						rc_div);
				at91_tc_write((tc + AT91_TC_RB), RC_VAL / 
						(rc_div * 2));
				at91_tc_write((tc + AT91_TC_CCR), AT91_TC_CLKEN | AT91_TC_SWTRG);
#endif
				slow_clock();
#if defined(CONFIG_SOM_9260_1K_TC) || defined(CONFIG_RDAC_CARRIER)
				at91_tc_write((tc + AT91_TC_RC), RC_VAL);
				at91_tc_write((tc + AT91_TC_RB), RC_VAL / 2);
				at91_tc_write((tc + AT91_TC_CCR), AT91_TC_CLKEN | AT91_TC_SWTRG);
#endif
				break;
			} else {
